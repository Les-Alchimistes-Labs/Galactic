...
public abstract class Personnage : Update
{
    ...
    protected Item.Item[]? _inventory = new Item.Item[8];
    
    
    public string Name => this._name;

    
    public Item.Item[]? Get_Inventory ()=> _inventory;


    public Item.Item? this[int i]
    {
        get
        {
            if (i < 0 || i > 0)
    ...
    public bool took(Item.Item item)
    {
        int i = 0;
        while (i<8)
        {
    ...
    public Item.Item trow(int i)
    {
        if (i < 0 || i > 0)
        {
            throw new IndexOutOfRangeException();
        }
        Item.Item? temp = _inventory[i];
        _inventory[i] = null;
        return temp;
